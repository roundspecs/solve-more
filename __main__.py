...
class User:
  def __init__(self,handle,solve_count,floored_rating) -> None:
    self.handle = handle
    self.solve_count = solve_count
    self.floored_rating = floored_rating
...
for i,info in enumerate(cf_api.user_info(handles=[user.handle for user in users])):
  users[i].floored_rating = (info.max_rating//100)*100

print("\nGood problems = {problem | problem.rating >= floor(user.max_rating)}\n")
...
for user in users:
  ...
  for submission in submissions:
    ...
    if problem.rating==None or problem.rating < user.floored_rating:
      continue
